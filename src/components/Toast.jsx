...
export default function Toast ({ message, type, onClose }) {
  ...
  return (
    <div className={`fixed max-lg:top-4 max-lg:left-1/2 max-lg:-translate-x-1/2 lg:bottom-6 lg:right-6 z-99 flex items-center gap-2 p-4 px-6 rounded-lg border shadow-lg overflow-hidden transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'} ${isSuccess ? 'bg-green-900 border-green-400 text-green-400' : 'bg-red-950 border-red-400 text-red-400'}`}>
      {isSuccess ? <CircleCheck size={20} color='currentColor' /> : <CircleX size={20} color='currentColor' />}
      <p className='text-sm text-center break'>{message}</p>
