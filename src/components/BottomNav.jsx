...
export default function BottomNav () {
  ...
  return (
    <nav className='fixed w-full bottom-0 flex md:hidden items-center justify-around border-t border-white/10 bg-surface h-14 z-99'>
      ...
      <Link to='/watchlist' onClick={requireAuth} className={pathname === '/watchlist' ? 'text-brand' : 'text-gray-400'}>
        <Bookmark size={24} />
      </Link>
      <Link to='/profile' onClick={requireAuth} className={pathname === '/profile' ? 'text-brand' : 'text-gray-400'}>
        <User size={24} />
      </Link>
